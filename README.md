...
`status` 返回 `INSTALLED_AND_PAIRED` 只说明本机入口已配对。具体任务还需区分：

| 结论 | 所需证据 |
|---|---|
| 已配置默认联用 | 本机指针和联用块有效 |
| 本任务已应用 | 代理已读取技能并据此评估或处理阶段接续 |
| 已准备交接 / 已请求切换 | 对应步骤确实完成；两者都不证明已切换 |
| 已切换并恢复 | 有本次实际切换证据，且已恢复规则与任务状态 |

短任务评估后继续完成即可，不必为证明使用而压缩。一次运行验收不能证明所有任务都自动使用；宿主自行切换也不能算作本技能主动触发。可以要求代理“核实这项任务是否实际应用，并说明本次是否发生切换”。
